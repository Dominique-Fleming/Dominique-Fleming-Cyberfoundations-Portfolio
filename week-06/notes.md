...
| `whoami` | Which user/account am I using? |
| `pwd` | Where am I in the filesystem? |
| `ip addr` | What is my IP address? |
| `ip route` | What path/gateway does my traffic use to get out? |
| `ping` | Can I reach this target and get a reply? |
| `traceroute` | What path does my traffic take to the target? |
| `dig` | What IP address does this name resolve to? |
| `curl` | Is the web/service actually responding with content? |
| `ssh` | Open a secure remote command-line session to another machine. |
| `exit` | Close the current shell/session and go back out. |
...
```
Getting TO a machine means the network path works and I can reach the right door. Getting INTO the machine means I also have to prove I’m allowed in with the right authentication. I saw this when SSH reached the machine and asked for a password — that proved I got TO it, but I still had to authenticate to get INTO it.
```
...
```
I proved the network was working by pinging a different VM on the same subnet and reaching a known-good address. This taught me that I can’t assume a silent ping means something is broken because some devices or gateways may not respond to ICMP even when the network is working.
```
...
```
My VM has a private IP address inside the VNet. NAT allows the VM to communicate out to the internet without needing its own public IP. When I need to reach the VM from outside the VNet, I use Azure Bastion, which acts like a guarded front desk and gives me a secure browser-based connection to the private VM.”
```
...
```
The VNet is like the whole building, while the subnet is like a floor or section inside the building. The VNet provides the overall private network, and the subnet divides it into smaller network ranges. Separating systems into smaller ranges can help security because it can limit how far an attack or incident can spread, making it easier to contain the problem
```
...
```
I start at the lower levels and work my way up, checking what works and using that evidence to rule out problems. The layers just help me identify exactly where the problem is happening instead of guessing.
```

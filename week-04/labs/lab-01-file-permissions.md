...
```
the owner can read and write, the group can read and write and everyone else can read and write
```
...
```
The owner can read and write on the shift-notes.txt file
```
...
```
The "other" audience is the most important to audit first because they usually are not normally allowed access to company privy information. "Others" could also include "attackers" or "terrorist groups".
```
...
```
The check before protects you from making changes to the wrong file. The check after protects you from not making changes to the correct or proper file.
```
...
```
For the master-inventory.txt file that gave all groups too much control and access to information that they probably would normally never need or use. So making sure the "owner" could still read and write would be needed. However the "group" aka "team" only needed to read what items they would help the "owner" with but not make changes since they would have to consult the "owner" probably. The "other" would never need to know about what the company would have on their master-inventory.txt file unless they meant to use it for harm or malicious intent. So I took away the write privilege from the "group" and took away the read and write privilege from "other".
...
```
Windows ACLs are useful when only one specific person needs access to a file, like someone in HR. The downside is that giving lots of people different permissions can get confusing and harder to manage.
```
...
![Permissions audit — final ls -l](https://raw.githubusercontent.com/Dominique-Fleming/Dominique-Fleming-Cyberfoundations-Portfolio/refs/heads/main/assets/screenshots/week-04/cli-permissions-audit.png)
